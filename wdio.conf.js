...
function browserFromEnv() {
  ...
  const [browserName, version, other] = process.env.BROWSER.split('-');

  const runWithoutJS = other === 'nojs';

  if (runWithoutJS && browserName !== 'firefox') {
    throw new Error(
      'We only support running Firefox without javascript at this time'
    );
  }
  ...
  if (runWithoutJS) {
    caps.profile = firefoxProfileWithJavaScriptDisabled;
  }
